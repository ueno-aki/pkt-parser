...
pub fn verify<CustomClaim:Serialize + DeserializeOwned>(chain: String, verified: &mut bool) -> Result<CustomClaim> {
    ...
    let key:ES384PublicKey = ES384PublicKey::from_der(&base64::decode(&header.x5u).unwrap()).unwrap();
...
pub fn verify_skin(public_key:&str,client:&str) ->Result<Value>{
    let key:ES384PublicKey = ES384PublicKey::from_der(&base64::decode(public_key).unwrap()).unwrap();
